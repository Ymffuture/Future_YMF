...
const Navbar = () => {
  ...
  return (
    <nav className="fixed w-full bg-[#FF0000] shadow z-50">
        ...
        <div className="hidden md:flex space-x-6 text-sm font-medium text-white">
          {menuItems.map((item) => (
            <div
              key={item.key}
              className={`${
                item.disabled ? 'text-gray-400 cursor-not-allowed' : 'hover:text-gray-200 hover:bg-white'
              }`}
            >
              {item.label}
            </div>
      ...
      {/* Mobile Drawer */}
      <Drawer
        title={<div className="text-gray-700 text-md text-center font-semibold">Menu</div>}
        placement="bottom"
        height="85%"
